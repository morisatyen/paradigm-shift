...
import { QrCode, X, Download, Printer, Eye, Star, Calendar, Users, Pencil, Trash2, Plus, Search, LayoutGrid, List } from "lucide-react";
...
const CollectionsGroups = () => {
  ...
  const filtered = COLLECTIONS.filter((collection) =>
    ...
    collection.museum.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <Input placeholder="Search collections..." value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9 w-72" />
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border text-left">
                {["Collection", "Museum", "Artworks", "Experiences", "Rating", "Votes", ""].map((header) => (
                  ...
                  <tr key={collection.id} className="border-b border-border last:border-0 hover:bg-muted/50">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <img src={collection.image} alt={collection.name} className="w-10 h-10 rounded object-cover" />
                        <div>
                    ...
                    <td className="p-4 text-sm text-muted-foreground">{collection.museum}</td>
                    <td className="p-4 text-sm text-foreground">{collection.artworks.length}</td>
                    <td className="p-4 text-sm text-foreground">{experiences.length}</td>
