...
const MuseumLayout = () => {
  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 px-4 py-8 space-y-6 overflow-y-auto scrollbar-hide">
        <section>
          <div className="px-3 pt-[14px] pb-[10px] text-[12px] tracking-[0.14em] font-bold uppercase text-sidebar-foreground/55">Workspace</div>
            ...
            {WORKSPACE_NAV.map((n) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 my-[1px] rounded-md text-[16px] font-medium transition-all duration-150 ease-in ${
                    active
                      ? "bg-sidebar-accent text-sidebar-accent-foreground"
                      : "text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent/45"
                  }`}
                >
                  <n.icon className={`w-5 h-5 ${active ? "text-[#E89A6B]" : "text-sidebar-foreground/55"}`} />
        ...
        <section>
          <div className="px-3 pt-[14px] pb-[10px] text-[12px] tracking-[0.14em] font-bold uppercase text-sidebar-foreground/55">Insights</div>
          <div className="space-y-1">
            {INSIGHTS_NAV.map((n) => {
              const active = isActivePath(n.to);
              return (
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-[300px] gradient-navy flex-col shrink-0 p-3">
        <SidebarContent />
      </aside>
      ...
      {/* Mobile sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-[280px] gradient-navy flex flex-col z-50 md:hidden p-3 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
