...
const MuseumLayout = () => {
  ...
  const resolvePageLabel = () => {
    ...
    const prefixed = allNav
      .filter((item) => item.to !== "/museum" && pathname.startsWith(item.to))
    ...
    if (pathname.includes("/collections-groups/")) return "Collections";
  ...
  const isActivePath = (to: string) =>
    to === "/museum/collections-groups"
      ? pathname.startsWith("/museum/collections-groups") ||
        pathname.startsWith("/museum/artworks") ||
        pathname.startsWith("/museum/experiences")
      : pathname === to || (to !== "/museum" && pathname.startsWith(to));

  const SidebarContent = () => (
    <>
      <div className="px-5 py-7 border-b border-sidebar-border/70 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2" onClick={() => setSidebarOpen(false)}>{SVG_LOGO}</Link>
        <button onClick={() => setSidebarOpen(false)} className="md:hidden text-sidebar-foreground/70 hover:text-sidebar-foreground"><X className="w-5 h-5" /></button>
      </div>
      <nav className="flex-1 px-4 py-8 space-y-6 overflow-y-auto scrollbar-hide">
        <section>
          <div className="px-3 pb-3 text-[13px] tracking-[0.18em] font-bold uppercase text-sidebar-foreground/55">Workspace</div>
            ...
            {WORKSPACE_NAV.map((n) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3.5 py-2.5 rounded-[10px] text-[17px] transition-colors ${
                    active
        ...
        <section>
          <div className="px-3 pb-3 text-[13px] tracking-[0.18em] font-bold uppercase text-sidebar-foreground/55">Insights</div>
          <div className="space-y-1">
            {INSIGHTS_NAV.map((n) => {
              const active = isActivePath(n.to);
              return (
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-64 gradient-navy flex-col shrink-0">
        <SidebarContent />
      </aside>
      ...
      {/* Mobile sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-64 gradient-navy flex flex-col z-50 md:hidden transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
