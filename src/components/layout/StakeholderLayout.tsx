...
const StakeholderLayout = () => {
  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 pl-2 pr-3 py-3 space-y-0.5 overflow-y-auto scrollbar-hide">
        {NAV.map(n => {
          const active = pathname === n.to || (n.to !== "/stakeholder" && pathname.startsWith(n.to));
          return (
            <Link key={n.to} to={n.to} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-[17px] transition-colors ${active ? "bg-sidebar-accent text-white" : "text-[#8FA3BA] hover:text-white hover:bg-sidebar-accent/50"}`}>
      ...
      <div className="p-4 border-t border-sidebar-border">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 rounded-full bg-sidebar-accent flex items-center justify-center text-sidebar-accent-foreground text-xs font-semibold">AP</div>
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-[326px] gradient-navy flex-col shrink-0 p-[6px]">
        <SidebarContent />
      </aside>
      ...
      {/* Mobile sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-[326px] gradient-navy flex flex-col z-50 md:hidden p-[6px] transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
