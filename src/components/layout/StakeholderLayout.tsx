...
const StakeholderLayout = () => {
    ...
    <>
      ...
      <nav className="flex-1 pl-2 pr-3 py-3 space-y-0.5 overflow-y-auto scrollbar-hide">
        {NAV.map(n => {
          ...
          return (
            <Link key={n.to} to={n.to} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-[13px] transition-colors ${active ? "bg-sidebar-accent text-white" : "text-[#8FA3BA] hover:text-white hover:bg-sidebar-accent/50"}`}>
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-[240px] gradient-navy flex-col shrink-0 p-[6px]">
        <SidebarContent />
      </aside>
      ...
      {/* Mobile sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-[240px] gradient-navy flex flex-col z-50 md:hidden p-[6px] transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
